...
function border_none(){
    $(".current_nav_btn").css("borderBottom","3px solid white");
...
//CHANGING THE SIGN IN PAGES
let ide;
function change_page(id,thisone){
    $("main").css({display:"none"});

    $(".section_btns").css({
        background: "linear-gradient(#A34CE9,#672C90)",
    ...
    $("#sign_in_page" + id).fadeIn(1000);
    $("#sign_in_page" + id).css("display","flex");
    ide = id
}
...
$(".continue_btn").click(function(){
    $("main").css({display:"none"});
    $("#sign_in_page" + (ide + 1)).fadeIn(1000);
    $("#sign_in_page" + (ide + 1)).css("display","flex");
})